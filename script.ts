...
(async () => {
	...
	for (let i = 0; i < accumulator.length; i++) {
		...
		const file = join(dir, ".codemodrc.json");
		const content = JSON.parse(await readFile(file, "utf-8"));
		if (content.owner) {
			content.name = `@${content.owner}/${content.name}`;
		} else {
			content.name = `@codemod.com/${content.name}`;
		}

		delete content.owner;
		delete content.meta?.changeType;
		delete content.meta?.timeSave;

		if (content.applicability) {
			if (content.applicability.length === 0) {
				delete content.applicability;
			} else {
				content.applicability = {
					from: content.applicability,
				};
			}
		}

		if (content.extensions) {
			content.include = content.extensions.map(
				(ext: string) => `**/*.${ext.slice(0, -1)}`,
			);
		}

		delete content.extensions;
		delete content.deps;

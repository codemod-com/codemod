...
import { buildRanges, type RangeCommand } from "@studio/utils/tree";
      ...
      ({
        ...
        setEditors: (editorsContents: EditorsSnippets[]) => {
          editorsContents.forEach((eC, i) => {
              get().addPair(eC.name);
              Object.entries(omit(['name'], eC))
                .forEach(([propName, editorContent]) => {
...
useSnippetsStore?.getState?.().setEditors(localStorage.getItem('editors') ? JSON.parse(localStorage.getItem('editors')) : [getSingleTestCase()])
useSnippetsStore?.getState?.().setEngine(localStorage.getItem('engine') as KnownEngines || "jscodeshift")
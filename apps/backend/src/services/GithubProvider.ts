...
let withPagination = async (
	...
	while (nextPage !== null) {
		let response = await paginatedRequest(nextPage);
		...
		if (
			...
			let nextUrl = linkHeader.match(nextPattern)?.[0];
			nextPage = nextUrl
...
export class GithubProvider implements SourceControlProvider {
	...
	private get __repoUrl() {
		let { owner, name } = parseGithubRepoUrl(this.__repo ?? '');

		return `${this.__baseUrl}/repos/${owner}/${name}`;
	}

	async createIssue(params: NewIssueParams): Promise<Issue> {
		let res = await axios.post(`${this.__repoUrl}/issues`, params, {
			headers: {
	...
	async createPullRequest(params: CreatePRParams): Promise<PullRequest> {
		let res = await axios.post(`${this.__repoUrl}/pulls`, params, {
			headers: {
				Authorization: this.__authHeader,
			},
		});
	...
	async getPullRequests(params: ListPRParams): Promise<PullRequest[]> {
		let queryParams = Object.entries(params).reduce<Record<string, string>>(
			(acc, [key, value]) => {
				if (value) {
					acc[key] = value;
				}
		...
		let res = await axios.get(`${this.__repoUrl}/pulls?${query}`, {
			headers: {
				Authorization: this.__authHeader,
			},
		});
	...
	async getAssignees(): Promise<Assignee[]> {
		let res = await axios.get(`${this.__repoUrl}/assignees`, {
			headers: {
				Authorization: this.__authHeader,
			},
		});

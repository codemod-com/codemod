...
let renderActionButtons = (
	...
	if (!codemodInProgress && !queued) {
		let handleDryRunClick = (e: React.MouseEvent) => {
			e.stopPropagation();
		...
		let handleCodemodLinkCopy = (e: React.MouseEvent) => {
			e.stopPropagation();
		...
		let handleCodemodArgumentsClick = () => {
			vscode.postMessage({
...
let Codemod = ({
	...
	let areButtonsVisible = focused || hovering;

...
let EXPANDABLE_CONTENT_MAX_HEIGHT = 1000;

let getIndent = (depth: number) => {
	return depth * 17;
};
...
let renderProgressBar = (progress: Progress | null) => {
	if (progress === null) {
		return null;
	}
...
let getCodemodNodeRenderer =
	({ autocompleteItems, progress, screenWidth }: Deps) =>
	({ nodeDatum, onFlip }: Props) => {
		let { node, focused, expanded, argumentsExpanded } = nodeDatum;
		let { hashDigest, label } = node;

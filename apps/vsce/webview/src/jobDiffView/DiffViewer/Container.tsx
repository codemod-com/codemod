...
HeaderProps) => {
	let jobKindText = getJobKindText(jobKind as unknown as JobKind);
	let hasDiff = diff !== null;
	let handleCopyFileName = (event: React.FormEvent<HTMLElement>) => {
	...
	let handleReviewedClick = (event: React.MouseEvent) => {
		event.stopPropagation();
...
let getJobKindText = (jobKind: JobKind): string => {
	switch (jobKind) {

...
export let syntheticErrorCodec = buildTypeCodec({
	kind: t.literal('syntheticError'),
	message: t.string,
});

export let workspaceStateCodec = t.union([
	buildTypeCodec({
		_tag: t.literal('Left'),
...
let buildCollectionCodec = <T extends t.Mixed>(entityCodec: T) => {
	return withFallback(
		buildTypeCodec({
...
export let panelGroupSettingsCodec = t.record(t.string, t.array(t.number));

export type PanelGroupSettings = t.TypeOf<typeof panelGroupSettingsCodec>;

export let persistedStateCodecNew = buildTypeCodec({

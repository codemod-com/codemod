...
export class DownloadService {
	...
	async downloadFileIfNeeded(
		...
		let lastModified = response?.headers['last-modified'] ?? null;
		let remoteModificationTime = lastModified
	...
	async #downloadFile(
		...
		let response = await retryingClient.get(url, {
			responseType: 'arraybuffer',
			'axios-retry': {
				retries: DEFAULT_RETRY_COUNT,
			},
		});
		let content = new Uint8Array(response.data);

...
let TYPE = 'codemodPanel';
let WEBVIEW_NAME = 'jobDiffView';

export let createBeforeAfterSnippets = (
	beforeContent: string,
	afterContent: string,
): { beforeSnippet: string; afterSnippet: string } => {
	let oldSourceFile = createInMemorySourceFile(
		'oldFileContent',
		beforeContent,
	);
	let sourceFile = createInMemorySourceFile('newFileContent', afterContent);

	let beforeNodeTexts = new Set<string>();
	let afterNodeTexts = new Set<string>();

	let diffObjs = diffTrimmedLines(beforeContent, afterContent, {
		newlineIsToken: true,
	});

	for (let diffObj of diffObjs) {
		if (!diffObj.added && !diffObj.removed) {
			continue;
		}
		let codeString = removeLineBreaksAtStartAndEnd(diffObj.value.trim());
	...
	let beforeSnippet = removeLineBreaksAtStartAndEnd(
		Array.from(beforeNodeTexts).join(''),
	);

	let afterSnippet = removeLineBreaksAtStartAndEnd(
		Array.from(afterNodeTexts).join(''),
	);
	return { beforeSnippet, afterSnippet };
};

let selectPanelViewProps = (
	...
	let focusedExplorerNodeHashDigest =
	...
	let tree = selectExplorerTree(state, rootPath);
	...
	let jobIndex = jobNodes.findIndex(
	...
	let job = mapPersistedJobToJob(persistedJob);

	let newFileTitle = job.newUri?.fsPath.replace(rootPath, '') ?? null;
	let oldFileTitle =
		[
			JobKind.moveFile,
...
export class CustomPanelProvider {
	...
	public constructor(
		...
		let listener = async () => {
			let nextViewProps = selectPanelViewProps(
				__mainWebviewViewProvider,
				__store.getState(),
				__rootPath,
			);
	...
	private async __upsertPanel(
		...
		if (this.__webviewPanel === null) {
			let webviewResolver = new WebviewResolver(this.__extensionUri);
			...
			this.__webviewPanel.webview.onDidReceiveMessage(
				async (message: WebviewResponse) => {
					...
					if (
						...
						let { beforeSnippet, afterSnippet } =

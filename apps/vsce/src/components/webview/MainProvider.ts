...
export let validateAccessToken = async (
	...
	try {
		let response = await axios.post<ValidateTokenResponse>(
...
export let createIssue = async (
	...
	let codemodRegistryRepoUrl = 'https://github.com/codemod-com/codemod';
	...
	let validation = createIssueResponseCodec.decode(data);
	...
	let decision = await window.showInformationMessage(
		'Github issue is successfully created.',
		'See issue in Github',
	);
	let { html_url } = validation.right;
...
export class MainViewProvider implements WebviewViewProvider {
	...
	constructor(
		...
		this.__store.subscribe(async () => {
			...
			let nextProps = this.__buildProps();
	...
	private async __getDirectoryPaths() {
		...
		let globPattern = buildGlobPattern(this.__rootUri, '/**');

		let directoryPaths = await glob(globPattern, {
		...
		let MAX_NUMBER_OF_DIRECTORIES = 10000;
	...
	private __onDidReceiveMessage = async (message: WebviewResponse) => {
		...
		if (message.kind === 'webview.codemodList.dryRunCodemod') {
			...
			let hashDigest = message.value;
			this.__store.dispatch(actions.setRecentCodemodHashes(hashDigest));

			let state = this.__store.getState().codemodDiscoveryView;
			let executionPath =
			...
			let uri = Uri.file(executionPath);
		...
		if (message.kind === 'webview.sourceControl.createIssue') {
			let accessToken = this.__userService.getLinkedToken();
	...
	public updateExecutionPath = async ({
		...
		let state = this.__store.getState().codemodDiscoveryView;
		let persistedExecutionPath = state.executionPaths[codemodHash];

		let oldExecutionPath = persistedExecutionPath ?? null;
		let newPathAbsolute = relativeToAbsolutePath(
			newPath,

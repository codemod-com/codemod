...
export let messageCodec = t.union([
	buildTypeCodec({
...
export let verboseEngineMessage = (message: EngineMessage): EngineMessage => {
	if (!('k' in message)) {
		return message;
	}
...
let CODEMOD_ENGINE_NODE_POLLING_INTERVAL = 5000;
let CODEMOD_ENGINE_NODE_POLLING_ITERATIONS_LIMIT = 100;
...
export class EngineService {
	...
	public async isCodemodEngineNodeLocated(): Promise<boolean> {
		...
		let childProcess = exec(command, { timeout: 3000 });

		if (childProcess.stdout === null) {
			return false;
		}

		let [latestVersion, userCLIVersion] = await Promise.all([
	...
	private async __fetchCLILatestVersion(): Promise<string> {
		let response = await axios.get(CLI_NPM_LINK);
	...
	private async __fetchCodemods(): Promise<void> {
		try {
			...
			let codemodListOrError = codemodListResponseCodec.decode(codemods);

			if (codemodListOrError._tag === 'Left') {
				let report = prettyReporter.report(codemodListOrError);
				throw new InvalidEngineResponseFormatError(report.join('\n'));
			}

			let codemodEntries =
				codemodListOrError.right.map(buildCodemodEntry);
	...
	async #onExecuteCodemodSetMessage(
		...
		let codemodName = message.command.name;
		...
		interfase.on('line', async (line) => {
			...
			let either = messageCodec.decode(JSON.parse(line));

			if (either._tag === 'Left') {
				let report = prettyReporter.report(either);

				console.error(report);
				return;
			}

			let message = verboseEngineMessage(either.right);
			...
			if (message.kind === 'create') {
				let newUri = Uri.file(message.newFilePath);
				let newContentUri = Uri.file(message.newContentPath);

				let hashlessJob: Omit<Job, 'hash'> = {
				...
				let oldUri = Uri.file(message.oldPath);
				let newContentUri = Uri.file(message.newDataPath);
				...
				let oldUri = Uri.file(message.oldFilePath);

...
export let jobHashCodec = t.brand(
	t.string,
...
export let persistedJobCodec = buildTypeCodec({
	hash: jobHashCodec,
	kind: t.union([
...
export let mapJobToPersistedJob = (job: Job): PersistedJob => {
	return {
		...job,
...
export let mapPersistedJobToJob = (persistedJob: PersistedJob): Job => {
	return {
		...persistedJob,

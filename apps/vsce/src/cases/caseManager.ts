...
export class CaseManager {
	...
	#onUpsertCasesMessage(message: Message & { kind: MessageKind.upsertCase }) {
		...
		let caseHashJobHashes = message.jobs.map(
	...
	#onAcceptCaseMessage(message: Message & { kind: MessageKind.acceptCase }) {
		...
		let caseHashJobHashSetManager = new LeftRightHashSetManager<
			CaseHash,
			JobHash
		...
		let jobHashes = caseHashJobHashSetManager.getRightHashesByLeftHash(
			message.caseHash,
		);
	...
	#onJobsAcceptedOrJobsRejectedMessage(
		...
		let cases = Object.values(state.case.entities).filter(
		...
		let removableCaseHashes: CaseHash[] = [];
		...
		for (let kase of cases) {
			...
			for (let job of message.deletedJobs) {
				let deleted = caseHashJobHashSetManager.delete(
					kase.hash,
					job.hash,
				);

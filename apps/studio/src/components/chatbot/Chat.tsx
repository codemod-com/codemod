...
const Chat = ({ id, className }: Props) => {
	...
	const initialMessages = useMemo(() => {
		try {
			...
			const stringifiedFrozenMessages =
				localStorage.getItem('frozenMessages');

			if (stringifiedFrozenMessages === null) {
				return [];
			}

			return parseFrozenMessages(
	...
	const {
		...
		onResponse(response) {
			...
			if (response.status === 500) {
				toast.error(
					'The server has encountered an error. Please retry later.',
				);
			}
		},
		headers: {
	...
	useEffect(() => {
		...
		const frozenMessages = messages.map((message) =>

...
export let openURL = (url: string): boolean => {
	// `spawnSync` is used because `execSync` has an input length limit
	let command = process.platform === 'win32' ? 'start' : 'open';
	let args = [url];
...
export let getCurrentUserData = async (): Promise<UserData | null> => {
	let [userCredentials] = await keytar.findCredentials('codemod.com');

	if (!isNeitherNullNorUndefined(userCredentials)) {
		return null;
	}

	let { account, password: token } = userCredentials;
...
export let initGlobalNodeModules = async (): Promise<void> => {
	let globalPaths = await Promise.allSettled([
...
export let getConfigurationDirectoryPath = (argvUnderScore?: unknown) =>
	join(
		String(argvUnderScore) === 'runOnPreCommit' ? process.cwd() : homedir(),
		'.codemod',
	);

export let rebuildCodemodFallback = async (options: {
	...
	let { globPattern, source, errorText, onSuccess, onFail } = options;

	let locateMainFile = async () => {
		let mainFiles = await glob(globPattern, {
			absolute: true,
			ignore: ['**/node_modules/**'],
			cwd: source,
			nodir: true,
...
export let oraCheckmark = chalk.green('✔');
export let oraCross = chalk.red('✖');

...
let consoleCallback = (consoleKind: ConsoleKind, message: string): void => {
...
let messageHandler = async (m: unknown) => {
	try {
		...
		try {
			...
			switch (initializationMessage.codemodEngine) {
				...
				case 'workflow':
					throw new Error(
						'Workflow engine is not supported in worker threads',
					);
				default:
					throw new Error(
...
export let executeWorkerThread = () => {

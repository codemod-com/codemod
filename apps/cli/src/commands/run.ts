...
let checkFileTreeVersioning = async (target: string) => {
	let force = true;

	try {
		let status = await execPromise('git status --porcelain', {
			cwd: target,
		});

		if (status.stdout.trim()) {
			let res = await inquirer.prompt<{ force: boolean }>({
				type: 'confirm',
				name: 'force',
				message:
...
export let handleRunCliCommand = async (options: {
	...
	let fileDownloadService = new FileDownloadService(args.cache, fs, printer);
	...
	if (codemodSettings.kind === 'runSourced') {
		...
		let engineOptions = await buildCodemodEngineOptions(
			codemod.engine,
			args,
		);
		...
		let engineOptions = buildCodemodEngineOptions(codemod.engine, args);
		...
		for (let preCommitCodemod of preCommitCodemods) {
			if (preCommitCodemod.source === 'package') {
				...
				let engineOptions = buildCodemodEngineOptions(
					codemod.engine,
					args,
				);
	...
	let runner = new Runner(codemodsToRun, fs, runSettings, flowSettings);
	...
	let executionErrors = await runner.run(
		async ({ codemod, commands, recipe }) => {
			let modifiedFilePaths = [
				...new Set(
					commands.map((c) =>
						'oldPath' in c ? c.oldPath : c.newPath,
					),
				),
			];

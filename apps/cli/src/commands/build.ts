...
let EXTERNAL_DEPENDENCIES = ['jscodeshift', 'ts-morph', '@ast-grep/napi'];

export let handleBuildCliCommand = async (options: {
	printer: PrinterBlueprint;
	source: string;
}) => {
	let { printer, source } = options;
	let absoluteSource = resolve(source);
	...
	let codemodRc = parseCodemodConfig(JSON.parse(codemodRcContent));

	let entryPointGlob = await glob(
		codemodRc.build?.input ? codemodRc.build.input : './src/index.{ts,js}',
		{ absolute: true, cwd: absoluteSource, nodir: true },
	);

	let entryPoint = entryPointGlob.at(0);
	...
	let buildOptions: Parameters<typeof esbuild.build>[0] = {
	...
	let { outputFiles } = await esbuild.build(buildOptions);

...
let EXTERNAL_DEPENDENCIES = ['jscodeshift', 'ts-morph'];

export let handleBuildCliCommand = async (
	...
	if (!isNeitherNullNorUndefined(entryPoint)) {
		...
		throw new Error(
			`Could not find entry point file in ${absoluteSource}.`,
		);
	}
	...
	let options: Parameters<typeof esbuild.build>[0] = {

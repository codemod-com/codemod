...
let CODEMOD_ENGINE_CHOICES: (KnownEngines | 'recipe')[] = [
	'jscodeshift',
	'recipe',
	'ts-morph',
	'filemod',
...
export let handleInitCliCommand = async (options: {
	...
	if (mainFilePath) {
		...
		let askedAnswers = await inquirer.prompt<{
		...
		}>([
			{
				...
				validate: (input) =>
					input === '' ? 'Name cannot be empty.' : true,
			},
			{
				type: 'list',
				name: 'engine',
		...
		answers = await inquirer.prompt([
			...
			// },
			{
				type: 'confirm',
				when: (answers) =>
					answers.engine !== 'ast-grep' &&
					answers.engine !== 'recipe',
				name: 'typescript',
	...
	if (mainFilePath) {
		let path = resolve(target, mainFilePath);
		try {

...
export let handlePublishCliCommand = async (options: {
	...
	try {
		...
		let isSourceAFile = await fs.promises
			.lstat(source)
		...
		if (isSourceAFile) {
			...
			let { mainPath } = await inquirer.prompt<{
				mainPath: string;
			}>({
				type: 'input',
				name: 'mainPath',
		...
		let resultPath = await handleInitCliCommand({
			printer,
			target: source,
	...
	if (!isSingleFile) {
		try {
			let descriptionMdBuf = await fs.promises.readFile(
	...
	let codemodRcData = codemodRcBuf.toString('utf-8');
	let codemodRc = parseCodemodConfig(JSON.parse(codemodRcData));
	...
	if (codemodRc.engine !== 'recipe') {
		...
		switch (codemodRc.engine) {
			...
			default:
				...
				if (codemodRc.build?.input) {
					let inputFiles = await glob(codemodRc.build.input, {
						absolute: true,
						cwd: source,
						nodir: true,
					});
					let entryPoint = inputFiles.at(0);
		...
		let mainFilePath = await rebuildCodemodFallback({
		...
		let mainFileBuf = await fs.promises.readFile(mainFilePath);

		formData.append(actualMainFileName, mainFileBuf);
	}

	let publishSpinner = printer.withLoaderMessage(
		chalk.cyan(

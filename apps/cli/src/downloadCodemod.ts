...
export class CodemodDownloader implements CodemodDownloaderBlueprint {
	...
	public async download(
		...
		let printableName = chalk.cyan.bold(doubleQuotify(name));
		...
		let { data, cacheUsed } = downloadResult;
		...
		try {
			...
			spinner?.fail();
			throw new Error(
				(err as Error).message ?? 'Error unpacking codemod',
			);
		}
		...
		if (config.engine === 'recipe') {
			...
			let { names } = await inquirer.prompt<{ names: string[] }>({
				name: 'names',
				type: 'checkbox',
				message:

...
export let executeMainThread = async () => {
	...
	let argvObject = buildGlobalOptions(yargs(slicedArgv));

	let argv = await Promise.resolve(argvObject.argv);
	...
	argvObject
		...
		.command(
			...
			(y) => buildGlobalOptions(y),
			async (args) =>
				executeCliCommand(() => handleLoginCliCommand(printer)),
		)
		.command(
			'logout',
			'logs out',
			(y) => buildGlobalOptions(y),
			async (args) =>
				executeCliCommand(() => handleLogoutCliCommand(printer)),
		)
		.command(
			'whoami',
			...
			(y) => buildGlobalOptions(y),
			async (args) =>
				executeCliCommand(() => handleWhoAmICommand(printer)),
		)
		.command(
			'build',
		...
		.command(
			...
			async (args) =>
				executeCliCommand(() =>
					handleInitCliCommand(printer, args.noPrompt),
				),
		);

...
let initializeDependencies = async (argv: {
	...
	let telemetryService: TelemetrySender<TelemetryEvent> = argv.telemetry
		? new PostHogSender({
	...
	argvObject
		...
		.command(
			...
			async (args) => {
				...
				return executeCliCommand(() =>
					handleLogoutCliCommand({ printer }),
				);
			},
		)
		.command(
		...
		.command(
			...
			async (args) => {
				...
				return executeCliCommand(async () => {
					let codemodConfig = await handlePublishCliCommand({
						printer,
						source: args.source ?? process.cwd(),
					});

...
let lockFilesToPmMap: Record<string, PackageManager> = {
...
export let handleInstallDependencies = async (options: {
	codemodName: string;
	...
	let { codemodName, printer, target, affectedFiles, deps } = options;
	...
	try {
		...
		while (true) {
			for (let lockFile of Object.keys(lockFilesToPmMap)) {
		...
		let packageJsons = await glob('**/package.json', {
			cwd: rootPath,
			ignore: ['**/node_modules/**'],
			absolute: true,
		});
		...
		if (detectedPackageManager === null) {
			for (let packageJsonPath of packageJsons) {
				let packageJsonContent = await readFile(packageJsonPath, {
					encoding: 'utf-8',
				});
				let packageJson = JSON.parse(packageJsonContent);

				if (packageJson.packageManager) {
					detectedPackageManager = packageJson.packageManager
						.split('@')
						.at(0);
					break;
				}
		...
		let affectedProjectsPackageJsons = packageJsons.filter((p) =>
		...
		let toDelete: string[] = [];
		for (let dep of deps) {
			let { libName } = extractLibNameAndVersion(dep);
		...
		let rootPackageJsonPath = join(rootPath, 'package.json');

		let installedDepsString = toInstall.length
			? chalk.green('+', toInstall.join('\n+ '))
			: '';
		let unInstalledDepsString = toDelete.length
			? chalk.red('-', toDelete.join('\n- '))
			: '';
		let affectedString = chalk.bold(
			affectedProjectsPackageJsons
				.map((p) => {
					let relativePath = relative(target, p);
		...
		if (defaultedToNpm) {
			let PM_INQUIRER_CHOICES = Object.values(lockFilesToPmMap).map(
				(pm) => ({
					name: pm,
					value: pm,
				}),
			...
			let { pm } = await inquirer.prompt<{ pm: PackageManager }>({
				type: 'list',
				name: 'pm',
				message:
					'Do you want to override the detected package manager? (npm)',
		...
		let removeCmd =
			detectedPackageManager === 'npm' ? 'uninstall' : 'remove';
		let addCmd = detectedPackageManager === 'npm' ? 'install' : 'add';
		...
		if (install === 'root') {
			let stopRemovalSpinner = printer.withLoaderMessage(
			...
			let stopInstallationSpinner = printer.withLoaderMessage(
			...
			for (let packageJsonPath of affectedProjectsPackageJsons) {
				let packageJsonContent = await readFile(packageJsonPath, {
					encoding: 'utf-8',
				});

...
export let buildSafeArgumentRecord = (
	codemod: Codemod,
	...
	let missing: typeof codemod.arguments = [];
	codemod.arguments.forEach((descriptor) => {
		let unsafeValue = argumentRecord[descriptor.name];
		...
		if (
			unsafeValue !== undefined &&
			typeof unsafeValue === descriptor.kind
		) {

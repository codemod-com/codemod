...
export class CodemodRunnerService {
	...
	public async run(codemodMetadata: CodemodMetadata): Promise<void> {
		let { codemodEngine } = codemodMetadata;
		...
		return new Promise<void>((resolve, reject) => {
			if (this.__status !== CodemodRunnerStatus.IN_PROGRESS) {
				...
				this.__process.on(
					'close',
					async (code) =>
						await this.__onCloseHandler(code, resolve, reject),
				);
			}
		});
	}
	...
	private __stdOutHandler = async (data: Buffer): Promise<void> => {
		...
		let progress = this.__extractStdOutProgress(message);
	...
	private __extractStdOutProgress = (
		...
		let match = message.match(/Processed (\d+) files out of (\d+)/);

		if (match && match.length >= 3) {
			let processed = Number(match[1]);

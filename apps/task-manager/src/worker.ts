...
let { TASK_MANAGER_QUEUE_NAME, CLERK_SECRET_KEY } = environment;
...
let worker = new Worker(
	...
	async (job: Job) => {
		let { id, name, data } = job;

		let auth = new AuthService(CLERK_SECRET_KEY);
		let token = await auth.getAuthToken(job.data.userId);

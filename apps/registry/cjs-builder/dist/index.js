...
export const buildCjs = async () => {
    ...
    if (relativeInputFilePath === undefined) {
        throw new Error('You must provide the relativeInputFileName to build the CJS bundle file');
    }
    ...
    const contents = outputFiles?.find((file) => file.path === absoluteOutputFilePath)
        ?.contents ?? null;
    if (contents === null) {
...
buildCjs()
    ...
    console.log('The bundled CommonJS contents have been written into %s', absoluteOutputFilePath);
})
    .catch((error) => {
    console.error(error);
});

...
async def generate_draft_codemod(before, after, codemod_path, prompts_thread, llm_engine, codemod_engine):
...
async def get_compiler_errors(codemod_path, codemod_engine):
...
async def attempt_error_correction(before_path, after_path, actual_after_path, compiler_errors, runtime_errors, codemod_alternatives, prompts_thread, llm_engine, codemod_engine, logger):
  if runtime_errors:
    ...
    await logger('in-progress', 'Attempting to fix runtime errors...')
    ...
    await logger('in-progress', 'Attempting to fix compiler errors...')
    ...
    await logger('in-progress', 'Attempting to fix unexpected output...')
...
async def test_generated_codemod(before_path, after_path, codemod_path, actual_after_path, codemod_engine):
...
async def generate_codemod(before_path, after_path, codemod_path, max_correction_attempts, llm_engine, codemod_engine, logger):
    ...
    for i in range(max_correction_attempts):
        ...
        await logger('in-progress', f'Attempt #{i+1} to refine the draft codemod...')
        # apply the codemod
        ...
        if is_same:
          if i == 0:
            await logger('finished', f'The draft codemod is already correct.')
          else:
            await logger('finished', f'Attempt #{i+1} corrected the codemod.')
          ...
          try:
            ...
            await logger('error', f'An error occurred while attempting to refine the codemod.')
    ...
    is_same, exec_errors, compiler_errors = test_generated_codemod(before_path, after_path, codemod_path, actual_after_path, codemod_engine)
    if is_same:
      await logger('finished', f'Attempt #{max_correction_attempts} corrected the codemod.')
    else:
      await logger('finished', f'Attempt #{max_correction_attempts} may have failed to correct the codemod.')
    return test_generated_codemod(before_path, after_path, codemod_path, actual_after_path, codemod_engine)

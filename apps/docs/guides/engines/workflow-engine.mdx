...
await git
  ...
  .files('**/*.ts') // find files using glob
	.js() // create js context for every found file inside directory context
...
await repoLink.files().js() // get all the JavaScript files in repository
...
<ResponseField name="files(glob, callback)">
  ...
  Calling files().js() - will find files with js, jsx, ts, tsx, cjs, mjs extensions.
...
<ResponseField name="js(callback)">

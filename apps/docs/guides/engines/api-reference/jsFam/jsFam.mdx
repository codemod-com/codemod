...
`jsFam` is always should be used together with [files](../fs/files) command. It automatically adds glob to find all js/ts files, so calling `files().jsFam()` is equivalent to `files('**/*.{js,jsx,ts,tsx,cjs,mjs}').jsFam()`.

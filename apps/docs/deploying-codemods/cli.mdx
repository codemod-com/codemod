...
```
<Accordion
  title="Running codemods without .codemodrc.json"
>
  By default, Codemod CLI will attempt using the engine specified in `.codemodrc.json`. However, if your local codemod does not follow the [required package structure](/codemod-registry/publishing-codemods#required-codemod-package-structure), you will need to specify the codemod engine manually.

  You can do so by using the `--engine` option. See example below: 
  ```bash

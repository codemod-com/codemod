...
The `search` command can be used to search for a codemods available in the Codemod Registry. This command uses the following format:
...
<ResponseField name="--raw" type="boolean" default="false">
  Can be used to disable prettier formatting to the files affected by the codemod.

```bash
codemod [codemod name] --raw [true/false]
```
...
The `sync` command can be used to sync local codemods with the [Codemod Registry](/codemod-registry/introduction). This command uses the following format:
...
The `login` command can be used to login to the Codemod platform. This command uses the following format:
...
The `logout` command can be used to logout from the Codemod platform. This command uses the following format:
...
The `publish` command can be used to publish a codemod to the Codemod Registry. Publishing codemods requires [logging in](#login-to-codemod-platform) to the Codemod platform. This command uses the following format:

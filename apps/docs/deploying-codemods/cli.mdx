...
The <Tooltip tip='Available aliases: `ls` and `search`'>`list`</Tooltip> command can be used to list the codemods available in Codemod Registry and optionally search for a specific codemod using its name or tags.
...
<Accordion title="Example">
  ...
  <Step title="List available codemods">
    Run `codemod list` to see all available codemods in Codemod Registry.
  </Step>
...
<ResponseField name="--source (-s)" type="string">
  Can be used to specify the path to the codemod package folder stored locally that you want to run using Codemod CLI.
...
The `publish` command can be used to [publish a codemod to Codemod Registry](/codemod-registry/publishing-codemods). Publishing codemods requires [logging in](#login-to-codemod-platform) to Codemod platform.
...
The `unpublish` command can be used to unpublish a codemod you own from Codemod Registry.

...
<video
  ...
  src="/images/deploying-codemods/cli/intuita-learn-workflow.mp4"
></video>
...
<ResponseField name="--source" type="string">
  ...
  ```bash
  codemod --source [path] --codemodEngine [codemod engine]
  ```

  <Note> Running local codemods requires specifying the codemod engine your codemod uses. You can do this by using the `--codemodEngine` option.</Note>
</ResponseField>

<ResponseField name="--codemodEngine" type="string">
...
<ResponseField name="--fileLimit" type="number" default="1000">
...
<ResponseField name="--usePrettier" type="boolean" default="false">
...
<ResponseField name="--useCache" type="boolean" default="false">
  ...
  <Tip>Enabling cache can help you save bandwidth and time for repetitive use of the same codemods. While disabling cache ensures you fetch the latest version of the codemod.</Tip>
</ResponseField>

<ResponseField name="--useJson" type="boolean" default="false">
  Can be used to switch the CLI responses to JSON format.

  ```bash
  codemod [codemod name] --useJson [true/false]
  ```
</ResponseField>

<ResponseField name="--threadCount" type="number" default="4">
...
<ResponseField name="--dryRun" type="boolean" default="false">
  ...
  ```bash
  codemod [codemod name] --dryRun [true/false]
  ```
</ResponseField>

...
### JavaScript engines (filemod, jscodeshift, ts-morph)
...
See how to build your first rule at [ast-grep documentation](https://ast-grep.github.io/guide/rule-config.html).
...
Recipes don't really have to contain input file, just the .codemodrc.json with codemod names that are included in the recipe specified under appropriate key.

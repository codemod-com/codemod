...
<Steps>
  ...
  <Accordion title="Error: The package `esbuild` could not be found.">
    This step requires global `esbuild` package installation. `esbuild` is installed by default while [installing Codemod CLI](/deploying-codemods/cli#installation). If you get an error saying "The package `esbuild` could not be found." try installing it manually by running `npm install -g esbuild`.
  </Accordion>

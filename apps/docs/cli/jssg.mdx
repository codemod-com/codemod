...
JSSG (JavaScript `ast-grep`) is a JavaScript/TypeScript codemod runner and testing framework built into the Codemod CLI. It enables you to write codemods in JavaScript and apply them to codebases with powerful CLI and test automation support.

## Running Codemods

To run a JavaScript codemod on a target directory, use the `codemod@next jssg run` command:

```bash
codemod@next jssg run my-codemod.js ./src --language javascript
```

### Options

<ResponseField name="--language <LANG>" type="string" required>
  Target language (e.g., `javascript`, `typescript`).
</ResponseField>
...
The JSSG Testing Framework provides comprehensive testing capabilities for JavaScript codemods using before/after fixture files.
...
2.  **Run the tests** using the `codemod@next jssg test` command.

    ```bash
    # Run tests for a codemod
    codemod@next jssg test my-codemod.js --language javascript

    # Update snapshots (creates or updates the `expected.js` files)
    codemod@next jssg test my-codemod.js --language javascript --update-snapshots
    ```

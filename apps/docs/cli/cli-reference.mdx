...
npx codemod <package-name> [key=value ...]
```
...
# Run with parameters (key=value as trailing args)
npx codemod @codemod-com/my-test-pkg version=latest target=src

# Specify a target directory
npx codemod @codemod-com/my-test-pkg -t /abs/path/to/repo version=latest
```

...
The fastest way to get started with Codemod CLI is to:

1. **Initialize a new codemod project**
...
For more details on building and running workflows, see the [Workflows documentation](/cli/workflows).

---

## Advanced Concepts

- **Workflows:** Orchestrate complex, multi-step codemod processes. [Learn more &rarr;](/cli/workflows)
- **jssg:** Run JavaScript/TypeScript codemods using the high-performance ast-grep engine. [See jssg section](#codemodnext-jssg)

---

# Command Reference

Below is a categorized reference of all Codemod CLI commands.

## Project Commands

- **`init`** — Initialize a new codemod project
- **`publish`** — Publish a workflow/codemod to a registry
- **`search`** — Search for packages in the registry
- **`run`** — Run a codemod from the registry

## Workflow Commands

- **`workflow`** — Manage and execute workflow YAMLs
...
- **`jssg`** — JavaScript ast-grep execution

...
<Steps>
  ...
  <Step title="Run your codemod">
    ...
    This runs your codemod workflow on your codebase.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Advanced Usage & Best Practices">
    ...
    - For large codebases, filter files with <code>
    ...
    - Use <code>
      ...
      --update-snapshots</code>

      to automatically update your expected outputs when your codemod changes.
    - See the <b>
      
      Testing Framework Details</b>
...
<Tip>
  Need a key? Generate one in the Codemod app <a href="https://app.codemod.com/api-keys" target="_blank">
...
<Info>
  Publishing from CI or on behalf of an organization? Install the <a href="https://github.com/apps/codemod">
  
  Codemod GitHub App</a>
...
<AccordionGroup>
  <Accordion title="Publishing from CI (w/ GitHub App)">
    <Note>
      Use this method when your organization has installed the <a href="https://github.com/apps/codemod">
      
      Codemod GitHub App</a>

      . The app injects <code>
      
      CODEMOD_TOKEN</code>

      automatically—no separate login step needed.
    </Note>
    ```yaml
    jobs:
      publish:
  ...
  <Accordion title="Publishing from CI (w/ API key)">
    <Note>
      Use this flow when the GitHub App isn’t installed. Requires <code>
      
      login --api-key</code>

      ; works for publishing new versions of existing codemods (the first publish must be interactive).
    </Note>
    <Frame>
      <APIKeyDemo />
    </Frame>

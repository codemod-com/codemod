...
<Tabs>
  ...
  <Tab title="Migration Recipes">
    Create a migration "recipe" by chaining Codemod Registry packages in a single workflow. No `.codemodrc.json` is needed.

    ```yaml codemod.yaml
    name: @your-scope/react-19-migration-recipe
    version: 1.0.0
    ...
    nodes:
      - id: migrate
        ...
        type: automatic
        steps:
          - name: Replace ReactDOM.render
            codemod:
              source: "react/19/replace-reactdom-render@latest"
          - name: Replace string refs
            codemod:
              source: "react/19/replace-string-ref"
          - name: Replace act import
            codemod:
              source: "react/19/replace-act-import"
          - name: Replace useFormState
            codemod:
              source: "react/19/replace-use-form-state"
          - name: Migrate prop-types to TypeScript
            codemod:
              source: "react/prop-types-typescript"
              # Optional per-codemod parameters (use key=value)
              args:
                - "dry_run=false"
    ```

    Run locally on a repo:
    ...
    Publish and run from the Registry:

    ```bash
    # From the package directory (with codemod.yaml + workflow.yaml)
    npx codemod publish

    # Later, run anywhere (key=value params are trailing args; use -t for target)
    npx codemod @your-scope/react-19-migration-recipe -t /abs/path/to/repo version=19
    ```

    Notes:
    - Steps execute in order; each `codemod.source` references a Registry package. Append `@<version>` to pin.
    - For `codemod.args`, prefer `key=value` pairs. They are forwarded to the inner workflow as parameters.
    - Use node/step `env:` if you need environment variables.
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Parameter flags" icon="check">
    Ability to pass parameters to workflows.
  </Step>

...
<Tabs>
  ...
  <Tab title="Migration Recipes">
    Create a migration recipe by chaining Registry codemods as shell steps. No `.codemodrc.json` is needed.
    ...
    nodes:
      - id: migrate
        name: Migration Recipe (chained codemods)
        type: automatic
        steps:
          - name: Step 1
    ...
    <Info>
      Parameters: Local workflows use `--param key=value`. Registry packages use trailing `key=value` (and `-t` for target). If a package requires the legacy runner, use `npx codemod@legacy <name> —target <path>`.
    </Info>
  </Tab>
</Tabs>

...
├── workflow.yaml
├── scripts/
│   └── my-script.sh      # Example shell script
└── tests/
    ├── input/
    └── expected/
```
...
├── workflow.yaml
├── rules/
│   └── config.js         # Example JS ast-grep rule
├── scripts/
└── tests/
    ├── input/
    └── expected/
...
├── workflow.yaml
├── rules/
│   └── config.yml        # Example YAML ast-grep rule
├── scripts/
└── tests/
    ├── input/
    └── expected/
```
    ...
    <Info>
    You can combine different codemod types in a single package by placing each type under its required directory (e.g., <code>scripts/</code> for shell scripts, <code>rules/</code> for ast-grep rules) and referencing them from <code>workflow.yaml</code>. This allows you to orchestrate complex, multi-step migrations using the best tool for each job.
    </Info>
  </Step>
...
nodes:
  - id: apply-js-rule
    name: Apply JS ast-grep Rule
    steps:
      - name: Run ast-grep JS rule
        ast-grep:
          include:
            - "**/*.js"
          base_path: "src"
          config_file: "rules/config.js"
```
...
nodes:
  - id: apply-yaml-rule
    name: Apply YAML ast-grep Rule
    steps:
      - name: Run ast-grep YAML rule
        ast-grep:
          include:
            - "**/*.ts"
            - "**/*.tsx"

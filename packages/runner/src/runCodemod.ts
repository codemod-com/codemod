...
export let buildPatterns = async (
	...
	let userExcluded = excludePatterns.map(formatFunc);
	let defaultExcluded = DEFAULT_EXCLUDE_PATTERNS.concat(
		DEFAULT_VERSION_CONTROL_DIRECTORIES,
	).map(formatFunc);
	let allExcluded = userExcluded.concat(defaultExcluded);
	...
	let { rootPath } = await getProjectRootPathAndPackageManager(
		flowSettings.target,
		true,
	);
	...
	if (rootPath !== null) {
		try {
			let gitIgnoreContents = await readFile(
	...
	if (codemod.engine === 'ast-grep') {
		...
		try {
			...
			let astGrepPatterns = astGrepLanguageToPatterns[config.language];
			if (!astGrepPatterns) {
	...
	let excludeFilterFunc = (p: string) => {
	...
	let exclude = allExcluded.filter(excludeFilterFunc);
	// remove everything that was excluded from the included patterns
	let include = formattedInclude.filter((p) => !exclude.includes(p));
...
function printRunSummary(
	...
	let { include, defaultExcluded, gitIgnoreExcluded, userExcluded, reason } =
		patterns;
...
export let runCodemod = async (
	...
	let allRecipeCommands: FormattedFileCommand[] = [];
	...
	if (codemod.engine === 'recipe') {
		...
		let patterns = await buildPatterns(flowSettings, codemod, null);

		let paths = await buildPathsGlob(fileSystem, flowSettings, patterns);
	...
	if (codemod.engine === 'workflow') {
		...
		let codemodSource = await readFile(codemod.indexPath, {
			encoding: 'utf8',
		});

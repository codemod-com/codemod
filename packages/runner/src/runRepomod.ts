...
export let runRepomod = async (
	...
	let unifiedFileSystem = new UnifiedFileSystem(
		buildPathHashDigest,
		getUnifiedEntry,
		globWrapper,
		readDirectory,
	...
	let processedPathHashDigests = new Set<string>();

	let totalPathHashDigests = new Set<string>();

	for (let path of filemod.includePatterns ?? []) {
	...
	let externalFileCommands = await executeFilemod(
		api,
		filemod,
		target,
		safeArgumentRecord,

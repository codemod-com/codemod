...
export let buildSurfaceAgnosticJob = (
	outputDirectoryPath: string,
	command: FormattedFileCommand,
): SurfaceAgnosticJob => {
	let jobHashDigest = randomBytes(20).toString('base64url');

	if (command.kind === 'createFile') {
		let dataUri = buildNewDataPathForCreateFileCommand(
			outputDirectoryPath,
			command,
		);
	...
	if (command.kind === 'updateFile') {
		let newDataUri = buildNewDataPathForUpdateFileCommand(
			outputDirectoryPath,
			command,
		);
...
export class SurfaceAgnosticCaseService {
	...
	public async emitPreamble(): Promise<void> {
		...
		let fileHandle = await this._fs.promises.open(

...
export class WorkerThreadManager {
	...
	private async __pullNewPath() {
		let iteratorResult = await this.__pathGenerator.next();
	...
	private async __work(): Promise<void> {
		...
		let filePath = this.__filePaths.shift();
		...
		let id = this.__idleWorkerIds.pop();
		...
		let data = await this.__getData(filePath);
	...
	private __buildOnWorkerMessage(i: number) {
		return async (m: unknown): Promise<void> => {
			let workerThreadMessage = decodeWorkerThreadMessage(m);

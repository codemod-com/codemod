...
export let repomod: Filemod<Dependencies, Options> = {
	includePatterns: [
		'**/package.json',
	...
	initializeState: async (options) => {
		...
		try {
			let json = JSON.parse(options.input);
	...
	handleData: async (
		...
		if (fileName.includes('ignore')) {
			...
			let filesToIgnore = parseIgnoreEntries(data);
		...
		if (fileName.includes('package.json')) {
			...
			let [, command] = getPackageManager(packageJson);

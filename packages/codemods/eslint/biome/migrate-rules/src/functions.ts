...
function deepCopy<T>(obj: T): T {
	...
	let newObj = (Array.isArray(obj) ? [] : {}) as T;
...
export function clearDependenciesAndAddNotes(
	...
	let newObj = deepCopy(packageJson);
...
export async function buildLinterConfig(
	...
	let biomeRules = await fetch(markDownUrl)
		.then((res) => res.text())
		.then((text) => text.split('\n').slice(5));

	let newObj = existingLinterConfig ? deepCopy(existingLinterConfig) : {};

	for (let [name, value] of Object.entries(rules)) {
		let ruleValue = (Array.isArray(value) ? value.at(0) : value) ?? 'error';
		let ruleName = name.split('/').at(-1);
		if (!ruleName) {
			continue;
		}

		let ruleIndex = biomeRules.findIndex((rule) =>
		...
		let biomePageContent = await fetch(
			`https://biomejs.dev/${urlFragment}`,
		).then((res) => res.text());

		let [, biomeRuleGroup, biomeRuleName] =
		...
		let biomeRuleValue: 'error' | 'warn' | 'off' =
			eslintToBiomeRuleValue(ruleValue);

		let existingRuleGroupDef =

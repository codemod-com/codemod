...
let tsconfigSchema = object({
	compilerOptions: optional(object({ types: optional(array(string())) })),
	include: optional(array(string())),
});

export let repomod: Filemod<Record<string, never>, Record<string, never>> = {
	includePatterns: [
		'**/package.json',
		'**/tsconfig.json',
	...
	handleFile: async (api, path, options) => {
		...
		let vitestContent = await api.readFile('vitest.config.ts');
		if (!vitestContent) {
			commands.push({
				kind: 'upsertFile',
      ...
      `;
		...
		if (path.endsWith('tsconfig.json')) {
			...
			if (tsconfigJson.compilerOptions?.types) {
				let newTypes = tsconfigJson.compilerOptions.types.filter(
					(type) => type !== 'mocha',
				);
			...
			if (tsconfigJson.include) {
				let newIncludes = tsconfigJson.include.filter(
					(type) => type !== 'mocha',
				);

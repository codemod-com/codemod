...
let resolveRequired = (path: NodePath) =>
...
function getTSType(path: NodePath) {
	...
	switch (name) {
		case 'func': {
			let restElement = j.restElement.from({
...
let isRequired = (path: NodePath) =>
	path.get('type').value === 'MemberExpression' &&
	path.get('property', 'name').value === 'isRequired';

function mapType(path: NodePath): TSType {
	let required = isRequired(path.get('value'));
	let key = path.get('key').value;
	let comments = path.get('leadingComments').value;
	let type = getTSType(
...
function getTSTypes(
	...
	let collected = [] as CollectedTypes;
	let propertyTypes = ['Property', 'ObjectProperty', 'ObjectMethod'];
...
function createInterface(path: NodePath, componentTypes: CollectedTypes) {
	...
	let types = componentTypes.find((t) => t.component === componentName);
	let typeName = `${componentName}Props`;
...
function addFunctionTSTypes(
	...
	source.forEach((path) => {
		let typeName = createInterface(path, componentTypes);
		if (!typeName) return;
...
let isOnlyWhitespace = (str: string) => !/\S/.test(str);
...
function guessTabWidth(source: string) {
	...
	for (let line of lines) {
		...
		let diff = Math.abs(indent - lastIndent);
...
export default function transform(file: FileInfo, api: API, opts: Options) {
	...
	let propTypes = collectPropTypes(source);

	let tsTypes = getTSTypes(
		propTypes,
		(path) => path.parent.get('left', 'object', 'name').value,
	);

	let staticPropTypes = collectStaticPropTypes(source);

	if (propTypes.length === 0 && staticPropTypes.length === 0) {
		return undefined;
	}

	let staticTSTypes = getTSTypes(
		staticPropTypes,

...
let buildRefAndPropsObjectPattern = (
	j: JSCodeshift,
	refArgName: string,
	propArgName: string,
) =>
...
let getForwardRefRenderFunction = (
	j: JSCodeshift,
	callExpression: CallExpression,
): FunctionExpression | ArrowFunctionExpression | null => {
	let [renderFunction] = callExpression.arguments;
...
export default function transform(file: FileInfo, api: API) {
	...
	root.find(j.CallExpression)
		...
		.replaceWith((callExpressionPath) => {
			let originalCallExpression = callExpressionPath.value;

			let renderFunction = getForwardRefRenderFunction(
				j,
			...
			let [propsArg, refArg] = renderFunction.params;

			if (
				!j.Identifier.check(refArg) ||
				!(
					j.Identifier.check(propsArg) ||
					j.ObjectPattern.check(propsArg)
				)
			) {
	...
	if (isDirty) {
		root.find(j.ImportDeclaration, {
			...
			let specifiersWithoutForwardRef =
				specifiers?.filter(
					(s) =>

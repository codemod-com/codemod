export default function transform(file, api) {
  ...
  root.find(j.ImportDeclaration).forEach(path => {
    if (j.Literal.check(path.node.source) && path.node.source.value === '@ethersproject/providers') {
  ...
  root.find(j.VariableDeclarator).forEach(path => {
    if (j.ObjectPattern.check(path.node.id) && path.node.init && j.Identifier.check(path.node.init) && path.node.init.name === 'providers') {
      path.node.id.properties.forEach(property => {
        if (j.Identifier.check(property.key) && property.key.name === 'InfuraProvider') {
          ...
          if (path.node.id.properties.length === 1) {
            ...
            path.node.id.properties = path.node.id.properties.filter(prop => prop.key.name !== 'InfuraProvider');
          }

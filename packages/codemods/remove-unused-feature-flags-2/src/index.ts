...
let FEATURE_FLAG_NAME = 'featureFlagName';
...
export let repomod: Filemod<Dependencies, State> = {
	...
	handleData: async (api, path, data, options, state) => {
		let marker =
			typeof options[FILE_MARKER] === 'string'
				? options[FILE_MARKER]
				: null;
		...
		let { featureFlagsExtracted, names: functionNames } = state;

		if (!featureFlagsExtracted && data.includes(marker)) {
			let root = jscodeshift.withParser('tsx')(data);

			let collection = root.find(jscodeshift.ExportNamedDeclaration);
			...
			collection.forEach(({ node }) => {
				...
				return node.declaration.declarations.forEach((declaration) => {
					...
					let [zerothArgument] = declaration.init.arguments;

					if (zerothArgument?.type !== 'ObjectExpression') {
						return false;
					}

					let [property] = zerothArgument.properties;
		...
		if (featureFlagsExtracted) {
			...
			for (let name of functionNames) {
				...
				root.find(jscodeshift.AwaitExpression, {

...
let selfContext = new AsyncLocalStorage<NodeContext>();

let parentContext = new AsyncLocalStorage<NodeContext>();

export let wrapContext = new AsyncLocalStorage<(...args: any[]) => any>();

let tree: { children: NodeContext[] } = {
	children: [],
};

export let getTree = () => tree;
...
export function fnWrapper<T extends (...args: any) => any>(
	...
	return ((...args: any[]) => {
		let parent = parentContext.getStore();
		let self = new NodeContext(name, fn.toString(), args);
		if (parent) {
...
> {
	...
	run(): R extends (...args: any[]) => Promise<any> | any
		...
		: void {
		let promise = new PLazy((resolve, reject) => {
			(async () => {
				await this.context()();
				let done = this._done?.();
				let res = await this._return?.(this);
				await done;
				return res;
			})()
				.then(resolve)
		...
		if (this._copyHelpersToPromise) {
			let wrappedHelpers = this.wrappedHelpers();
			for (let key in wrappedHelpers) {

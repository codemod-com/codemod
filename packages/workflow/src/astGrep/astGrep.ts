...
let runExternalAstGrepRule = async (rule: any, filepath: string) => {
	let packageJsonFilepath = require.resolve('@ast-grep/cli/package.json');
	let { bin: binaries } = JSON.parse(
				...
				for (let node of nodes) {
					if (next) {
						...
						await astGrepNodeContext.run(
							...
							callback
								? astGrepHelpers
								: self.wrapHelpers(astGrepHelpers),
						);
					}
				}
...
export let astGrep = fnWrapper('astGrep', astGrepLogic);

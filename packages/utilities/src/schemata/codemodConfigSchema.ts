...
let getFirstValibotIssue = (issues: Issues) => {
	...
	for (let issue of issues) {
		...
		let firstIssueWithPath = issues.find((issue) =>
...
export let extractLibNameAndVersion = (val: string) => {
	let parts = val.split('@');
...
export let codemodNameRegex = /[a-zA-Z0-9_/@-]+/;

export let argumentsSchema = array(
	union(
		[
			object({
				name: string(),
...
export let piranhaLanguageSchema = union(
...
let versionUnion = union(
	[literal('<'), literal('>'), literal('='), literal('<='), literal('>=')],
	'Invalid version range operator.',
);

let semVerValidationFunc = (val: string) =>
	!!semver.valid(val) ||
	val === '*' ||
	val === 'latest' ||
	val === 'next' ||
...
let getLibraryVersionTupleValidator = (msg: string) =>
	tuple(
		[
			...
			union([
				...
				string([
					custom(
						semVerValidationFunc,
						`"version" has to be a valid semver.`,
					),
				]),
...
export let knownEnginesSchema = union(
	knownEngines,
	'Specified engine is not supported.',
);
export type KnownEngines = Output<typeof knownEnginesSchema>;

let allEngines = [...knownEngines, literal('recipe'), literal('piranha')];
export let allEnginesSchema = union(
	allEngines,
	'Specified engine is not supported.',
);
export type AllEngines = Output<typeof allEnginesSchema>;

let configJsonBaseSchema = object({
	...
	deps: optional(
		array(
			string([
				custom((val) => {
					let { libName, version } = extractLibNameAndVersion(val);
					// e.g. -jest
		...
		object(
			{
				...
				output: optional(
					string('Build output path has to be a string.'),
				),
			},
...
export let codemodConfigSchema = union([
	merge([
...
export let parseCodemodConfig = (config: unknown) => {
	try {

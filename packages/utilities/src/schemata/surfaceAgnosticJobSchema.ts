...
let NEW_FILE_CREATED = 0;
let OLD_FILE_CHANGED = 1;
let OLD_FILE_DELETED = 2;
let OLD_FILE_COPIED = 3;

let PATH_CHANGED = 1 << 2; // when OLD_FILE_CHANGED set
let DATA_CHANGED = 1 << 3; // when OLD_FILE_CHANGED set
...
let surfaceAgnosticJobSchema = union([
	object({
...
export let parseSurfaceAgnosticJob = (input: unknown) =>

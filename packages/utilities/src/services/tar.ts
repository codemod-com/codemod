...
export class TarService {
	...
	public async unpack(
		...
		let bufferMap = new Map<string, ReadonlyArray<Buffer>>();
		...
		await new Promise<void>((resolve, reject) => {
			...
			let conditionalResolve = () => {
				if (finished && remainingEntryCount === 0) {
					resolve();
				}
			};

			let entryHandler = (entry: tar.ReadEntry): void => {
				if (entry.type !== 'File') {
					return;
				}
				...
				let dataHandler = (data: Buffer): void => {
					let buffers = bufferMap.get(entry.path)?.slice() ?? [];
					buffers.push(data);
		...
		for (let [path, buffers] of bufferMap) {
			let absolutePath = join(rootDirectoryPath, path);
	...
	public async pack(buffers: { name: string; data: Buffer }[]) {
		...
		let collectStream = bl((err, buffer) => {
			if (err) throw err;

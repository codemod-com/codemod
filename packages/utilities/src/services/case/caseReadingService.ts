...
let buildCase = (outerCase: OuterCase): SurfaceAgnosticCase => {
	let innerDataHashDigest = createHash('ripemd160')
	...
	let caseHashDigest = outerCase.innerData
		.subarray(0, 20)
		.toString('base64url');
	let codemodHashDigest = outerCase.innerData
		.subarray(20, 40)
		.toString('base64url');

	let createdAt = outerCase.innerData.subarray(40, 48).readBigInt64BE();

	let pathByteLength = outerCase.innerData.subarray(48, 50).readUint16BE();

	let recordByteLengthStart = 50 + pathByteLength;

	let absoluteTargetPath = outerCase.innerData
		.subarray(50, recordByteLengthStart)
		.toString();

	let recordByteLength = outerCase.innerData
		.subarray(recordByteLengthStart, recordByteLengthStart + 2)
		.readUint16BE();

	let record = outerCase.innerData
		.subarray(
	...
	let argumentRecord = parseArgumentRecordSchema(JSON.parse(record));
...
let buildJob = (outerJob: OuterJob): SurfaceAgnosticJob => {
	let innerDataHashDigest = createHash('ripemd160')
	...
	let jobHashDigest = outerJob.innerData
		.subarray(0, 20)
		.toString('base64url');

	let kind = outerJob.innerData.subarray(20).readUInt8();

	let readUris = (bufferCount: number) => {
		let uris: string[] = [];
		let start = 21;
		let end = 23;

		for (let i = 0; i < bufferCount; ++i) {
			let byteLength = outerJob.innerData
				.subarray(start, end)
				.readUint16BE();
	...
	if (kind === JOB_KIND.CREATE_FILE) {
		let [pathUri, dataUri] = readUris(2);
	...
	if (kind === JOB_KIND.UPDATE_FILE) {
		let [pathUri, newDataUri] = readUris(2);
	...
	if (kind === JOB_KIND.MOVE_FILE) {
		let [oldPathUri, newPathUri] = readUris(2);
	...
	if (kind === JOB_KIND.MOVE_AND_UPDATE_FILE) {
		let [oldPathUri, newPathUri, newDataUri] = readUris(3);
	...
	if (kind === JOB_KIND.COPY_FILE) {
		let [sourcePathUri, targetPathUri] = readUris(2);
...
let getByteLength = (state: State): number => {
...
let read = (buffer: Buffer, state: State): StateRecipe => {
	if (state.position === POSITION.BEFORE_PREAMBLE) {
		if (
			Buffer.compare(
				buffer.subarray(0, 4),
	...
	if (
		...
		try {
			let surfaceAgnosticCase = buildCase({
				...state.outerCase,
				innerData: buffer,
			});
	...
	if (
		...
		try {
			let surfaceAgnosticJob = buildJob({
				...state.outerJob,
				innerData: buffer,
			});
...
export class CaseReadingService extends EventEmitter {
	...
	public async initialize() {
		let byteLength = getByteLength(this._state);
	...
	protected _read(buffer: Buffer) {
		let stateRecipe = read(buffer, this._state);
		...
		if ('event' in stateRecipe && stateRecipe.event === 'end') {
			...
			this._fileReadingService
				.close()
				.then(() => {
					let hashDigest = this._hash.digest();

...
export let createHashDigest = (buffer: Buffer): Buffer =>
	createHash('ripemd160').update(buffer).digest();

let buildUint8Buffer = (value: number): Buffer => {
	if (value > 0xff - 1) {
...
let buildUint16Buffer = (value: number): Buffer => {
	if (value > 0xffff - 1) {
...
let buildBigIntBuffer = (bi: bigint): Buffer => {
...
let buildStringBuffer = (str: string): Buffer => {
	let MAXIMUM_LENGTH = 16 * 1024 - 1;
	...
	let lengthBuffer = buildUint16Buffer(stringBuffer.byteLength);

	return Buffer.concat([lengthBuffer, stringBuffer]);
};

export let serializePreamble = (): Buffer =>
	Buffer.concat([
		Buffer.from([0xaa, 0xbb, 0xcc, 0xdd]),
		new Uint8Array([1, 0, 0, 0]),
	]);

export let serializeCase = (kase: SurfaceAgnosticCase): Buffer => {
	let innerBuffer = Buffer.concat([
	...
	let hashDigest = createHashDigest(innerBuffer);
...
export let serializeJob = (job: SurfaceAgnosticJob): Buffer => {
	let buffers: Buffer[] = [
	...
	let innerBuffer = Buffer.concat(buffers);
...
export let serializePostamble = (hashDigest: Buffer) =>
...
export class CaseWritingService extends EventEmitter {
	...
	public async writeCase(kase: SurfaceAgnosticCase) {
		try {
			...
			let outerBuffer = serializeCase(kase);
